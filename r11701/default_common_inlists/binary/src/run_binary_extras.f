...
! ***********************************************************************
      ...
      subroutine my_CE_init(binary_id, restart, ierr)
         ...
         ! not a priority, but needs to be sorted out whenever double
         ! core
         ...
         if (.not. restart) then
            ...
            ! get energy from the EOS and adjust the different
            ! contributions from recombination/dissociation
            allocate(adjusted_energy(s% nz))
            do k=1, s% nz
               ! the following lines compute the fractions of HI, HII,
               ! HeI, HeII and HeIII
               ! things like ion_ifneut_H are defined in
               ! $MESA_DIR/ionization/public/ionization.def
               ! this file can be checked for additional ionization
               ! output available
               frac_HI = get_ion_info(s,ion_ifneut_H,k)
               frac_HII = 1 - frac_HI

               ! ionization module provides neutral fraction and average
               ! charge of He.
               ! use these two to compute the mass fractions of HeI and
               ! HeII
               ...
               if(.false.) then
                  ! for debug, check the mismatch between the EOS energy
                  ! and that of a gas+radiation
            ...
            ! Compute internal and potential energies from the inside
            ! out, and in the opposite direction.
            ! Here I abuse the variable for internal energy by adding
            ! rotational kinetic energy as well
